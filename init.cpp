...
void init() {
	...
	// 把relation2id.txt的第一个数字(总数)赋给relationTotal
	fclose(fin);

    // 分配内存，把calloc返回的指针确定为int指针
	...
	fclose(fin);
    // sort(begin, end, 准则 cmp) begin end 均为指针
	...
	// memset 初始化目标，初始化的值设为-1. 初始化的长度设为全部长度
	// lefHead跟lefTail均被初始化为0
	...
	// leftail和rigtail分别记录了不同实体作为tail出现的位置的起始点和结束点。
	//没出现的起始被初始化为0，结束被初始化为-1.

	//有了上述起始跟结束，下面探讨关系下的平均。对于某一个relation，计算左实体跟右实体出现的平均次数
	// 等于 （关系出现的总次数）/（关系左（右）边的总实体数）
